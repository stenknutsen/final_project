...
<body>
	...
	<sql:query dataSource="${snapshot}" var="result"> SELECT * FROM game, item, auction WHERE game.title = '<%=session.getAttribute("game")%>' AND item.item_condition = '<%=session.getAttribute("condition")%>' AND game.upc_code = item.upc_code AND item.item_id = auction.item_id; </sql:query>
